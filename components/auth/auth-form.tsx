...
export function AuthForm() {
  ...
  /* helpers */
  const handleInput =
    (form: "signup" | "signin", field: string) =>
    (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      if (form === "signup") {
        setSignUpValues((s) => ({
          ...s,
          [field]: field === "age" ? Number(val) || 0 : val,
        }));
      } else {
  ...
  const submitSignIn = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast({
        title: "Welcome Back! 💄",
        description:
          "You have been signed in successfully. Get ready to transform your look!",
      });
    } catch (err: any) {
    ...
    <div className="flex items-center justify-center min-h-screen bg-secondary">
      <Card className="w-full max-w-md shadow-xl border-0 bg-background/95 backdrop-blur-sm">
        <CardHeader className="text-center space-y-4">
          ...
          <div>
            <CardTitle className="text-3xl font-bold text-signature hover:opacity-90">
              Glamour Gaze
            </CardTitle>
          ...
          >
            ...
            <TabsContent value="signin" className="mt-6">
              <form onSubmit={submitSignIn} className="space-y-4">
                ...
                <Button
                  type="submit"
                  className="w-full bg-fuchsia-gradient text-white font-medium py-2.5"
                  disabled={loading}
                >
