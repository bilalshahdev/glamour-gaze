...
export class MakeupRenderer {
  ...
  drawMakeup(image: HTMLImageElement, landmarks: FaceLandmarks, config: MakeupConfig): void {
    ...
    if (config.cheeks) {
      ...
      this.drawCheekMakeup(landmarks.leftCheek, landmarks.rightCheek, config.cheeks)
    }
    ...
    if (config.eyebrows) {
      ...
      this.drawEyebrowMakeup(landmarks.leftEyebrow, landmarks.rightEyebrow, config.eyebrows)
    }
  ...
  private drawLipMakeup(lipPoints: number[][], config: any): void {
    ...
    if (lipPoints.length > 0) {
      ...
      if (config.gloss) {
        ...
        const centerX = lipPoints.reduce((sum, p) => sum + p[0], 0) / lipPoints.length
        const centerY = lipPoints.reduce((sum, p) => sum + p[1], 0) / lipPoints.length

        const gradient = this.ctx.createRadialGradient(centerX, centerY - 3, 0, centerX, centerY - 3, 20)
  ...
  private drawEyeMakeup(leftEyePoints: number[][], rightEyePoints: number[][], config: any): void {
    console.log("Drawing eye makeup with points:", { leftEyePoints, rightEyePoints, config })
    ...
    eyeGroups.forEach((eyePoints, index) => {
      ...
      const centerX = eyePoints.reduce((sum, p) => sum + p[0], 0) / eyePoints.length
      const centerY = eyePoints.reduce((sum, p) => sum + p[1], 0) / eyePoints.length

      console.log(`Eye ${index + 1} center:`, { centerX, centerY })

      // Create eyeshadow area (larger than eye for realistic effect)
      const gradient = this.ctx.createRadialGradient(centerX, centerY - 10, 0, centerX, centerY - 10, 40)
      ...
      if (config.shimmer) {
        ...
        const shimmerGradient = this.ctx.createRadialGradient(centerX, centerY - 8, 0, centerX, centerY - 8, 25)
  ...
  private drawCheekMakeup(leftCheekPoints: number[][], rightCheekPoints: number[][], config: any): void {
    console.log("Drawing cheek makeup with points:", { leftCheekPoints, rightCheekPoints, config })

    if (!leftCheekPoints || !rightCheekPoints || leftCheekPoints.length === 0 || rightCheekPoints.length === 0) {
    ...
    cheekGroups.forEach((cheekPoints, index) => {
      ...
      const centerX = cheekPoints.reduce((sum, p) => sum + p[0], 0) / cheekPoints.length
      const centerY = cheekPoints.reduce((sum, p) => sum + p[1], 0) / cheekPoints.length

      console.log(`Cheek ${index + 1} center:`, { centerX, centerY })

      // Create soft circular blush with better blending
      const gradient = this.ctx.createRadialGradient(centerX, centerY, 0, centerX, centerY, 50)
      gradient.addColorStop(0, config.color)
  ...
  private drawEyebrowMakeup(leftBrowPoints: number[][], rightBrowPoints: number[][], config: any): void {
    console.log("Drawing eyebrow makeup with points:", { leftBrowPoints, rightBrowPoints, config })

    if (!leftBrowPoints || !rightBrowPoints || leftBrowPoints.length === 0 || rightBrowPoints.length === 0) {
    ...
    browGroups.forEach((browPoints, index) => {
      ...
      console.log(`Drawing eyebrow ${index + 1} with ${browPoints.length} points`)
  ...
  private drawHairMakeup(facePoints: number[][], config: any): void {
    ...
    const hairlinePoints = facePoints.filter((p) => p[1] < topY + 150).sort((a, b) => a[0] - b[0])
    ...
    for (let i = 0; i < numStrands; i++) {
      ...
      if (y < topY + 100) {
        ...
        this.ctx.strokeStyle = this.adjustColorBrightness(config.color, Math.random() * 40 - 20)
